...
class _ConnectivityPageState extends State<ConnectivityPage> {
  ...
  bool _loading = false;
  String _lastRobotIp = '';
  String _savedWifiSsid = '';
  String _savedBluetoothName = 'AK-1';
  bool _savedBluetoothEnabled = false;
  ...
  Future<void> _loadSavedSettings() async {
    ...
    if (!mounted) return;
    setState(() {
      _savedWifiSsid = prefs.getString('ak1_wifi_ssid') ?? '';
      _savedBluetoothName = prefs.getString('ak1_bt_name') ?? 'AK-1';
      _savedBluetoothEnabled = prefs.getBool('ak1_bt_enabled') ?? false;
    });
  }

  Future<void> _saveWifiLocally(String ssid) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('ak1_wifi_ssid', ssid);
  }

  Future<void> _saveBluetoothLocally(bool enabled, String name) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool('ak1_bt_enabled', enabled);
    await prefs.setString('ak1_bt_name', name);
  }
  ...
  Future<void> _readRobotStatus() async {
    ...
    try {
      ...
      setState(() {
        ...
        _lastRobotIp = data['ip']?.toString() ?? '';
        if (_lastRobotIp.isNotEmpty && !_ipController.text.contains('http')) {
          _ipController.text = _lastRobotIp;
        }
      });
    } catch (_) {
      if (mounted) {
  ...
  Future<void> _configureWifi() async {
    ...
    await _saveWifiLocally(result['ssid']?.toString() ?? '');
    if (mounted) setState(() => _savedWifiSsid = result['ssid']?.toString() ?? '');
    await _postRobot('/wifi/config', result, successMessage: 'تنظیم Wi-Fi در برنامه ذخیره شد. اگر ربات در دسترس باشد، تنظیمات برای ESP32-CAM هم ارسال می‌شود.');
  }
  ...
  Future<void> _configureBluetooth() async {
    ...
    final name = result['name']?.toString() ?? 'AK-1';
    await _saveBluetoothLocally(enabled, name);
    if (mounted) setState(() { _savedBluetoothEnabled = enabled; _savedBluetoothName = name; });
    await _postRobot('/bluetooth/config', result, successMessage: 'تنظیم Bluetooth در برنامه ذخیره شد. اگر ربات در دسترس باشد، برای ESP32-CAM هم ارسال می‌شود.');
  }

  Future<Map<String, dynamic>?> _wifiDialog() async {
    final ssidController = TextEditingController(text: _savedWifiSsid);
  ...
  Future<Map<String, dynamic>?> _bluetoothDialog() async {
    bool enabled = _robotBluetooth.toLowerCase().contains('on') || _robotBluetooth.contains('فعال') || _savedBluetoothEnabled;
    final nameController = TextEditingController(text: _savedBluetoothName);
    return showDialog<Map<String, dynamic>>(
      context: context,
  ...
  Future<void> _postRobot(String path, Map<String, dynamic> payload, {String? successMessage}) async {
    if (_baseUrl.isEmpty) return;
    try {
      final client = HttpClient();
      final request = await client.postUrl(Uri.parse('$_baseUrl$path')).timeout(const Duration(seconds: 5));
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        String message = successMessage ?? 'درخواست برای ربات ارسال شد.';
        try {
          final data = jsonDecode(body) as Map<String, dynamic>;
          ...
          if (path == '/bluetooth/config' && data['name'] != null) {
            _savedBluetoothName = data['name'].toString();
          }
        } catch (_) {}
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('تنظیمات در برنامه ذخیره شد؛ ربات فعلاً در آدرس $_baseUrl پاسخ نداد. وقتی Firmware روی ESP32-CAM نصب و ربات به شبکه وصل شد، دوباره «اتصال و دریافت وضعیت» را بزن.')),
        );
      }
    }
  }
      ...
      child: Scaffold(
        ...
        body: ListView(
          ...
          children: [
            ...
            Card(child: ListTile(leading: const Icon(Icons.wifi_rounded), title: const Text('Wi-Fi ربات'), subtitle: Text(_robotWifi == 'نامشخص' && _savedWifiSsid.isNotEmpty ? 'ذخیره‌شده: $_savedWifiSsid' : _robotWifi), trailing: FilledButton(onPressed: _configureWifi, child: const Text('تنظیم')))),
            Card(child: ListTile(leading: const Icon(Icons.bluetooth_rounded), title: const Text('Bluetooth ربات'), subtitle: Text(_robotBluetooth == 'نامشخص' ? (_savedBluetoothEnabled ? 'ذخیره‌شده: روشن — $_savedBluetoothName' : 'ذخیره‌شده: خاموش') : _robotBluetooth), trailing: FilledButton(onPressed: _configureBluetooth, child: const Text('تنظیم')))),
            const SizedBox(height: 8),
            const Card(child: Padding(padding: EdgeInsets.all(16), child: Text('تنظیمات Wi-Fi و Bluetooth ابتدا داخل خود اپ ذخیره می‌شوند. برای اعمال آن‌ها روی ربات، ESP32-CAM باید روشن باشد و Firmware ارتباطی AK-1 روی آن نصب شده باشد.'))),
          ],
        ),
      ),
    );
